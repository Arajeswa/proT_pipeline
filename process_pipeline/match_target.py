...
def match_target(dataset_id)->None:
    ...
    df_trg = df_trg[df_trg[trans_batch_label].isin(df_input_short.index)] # filter out unavailable batches


    df_query = df_trg[[target_id_label, trans_batch_label]].drop_duplicates().set_index(target_id_label)


    df_list = []  # temporary holder for all sub-DataFrames

    for idx in tqdm(df_query.index):
        batch = df_query.loc[idx, trans_batch_label]
