...
def match_target(dataset_id)->None:
    ...
    df_input_short = pd.read_parquet(join(OUTPUT_DIR,trans_df_input_short)).set_index(trans_group_id)
    df_trg = pd.read_csv(filepath_target, sep=target_sep)
    df_trg = df_trg[df_trg[trans_group_id].isin(df_input_short.index)] # filter out unavailable batches

    # different id for the same batch
    if target_id_label in df_trg.columns:
        
        df_query = df_trg[[target_id_label, trans_group_id]].drop_duplicates().set_index(target_id_label)

        df_list = []  # temporary holder for all sub-DataFrames

        for idx in tqdm(df_query.index):
            batch = df_query.loc[idx, trans_group_id]
    ...
    # mean over batches
        ...
        df_query = df_trg[trans_group_id].unique()
        df_list = []  # temporary holder for all sub-DataFrames
        
        for batch in tqdm(df_query):

            # Copy the slice once
            df_temp = df_input_short.loc[batch].reset_index().copy()
            df_temp[trans_group_id] = batch
